...
class Cell():
    ...
    def __init__(self, n_crit, l):
        ...
        self.l = l            # orden de los esféricos armónicos
        self.multipole = numpy.zeros(l**2, dtype=numpy.complex_)  # multipole array
...
def add_child(octant, p, cells, n_crit):
    ...
    cells.append(Cell(n_crit, cells[0].l))
...
def build_tree(particles, root, n_crit):
    ...
    for i in range(n):
        ...
        while cells[curr].nleaf >= n_crit:
            cells[curr].nleaf += 1
            octant = (particles[i].x > cells[curr].x) \
                   + ((particles[i].y > cells[curr].y) << 1) \
...
def get_particles(p, cells, n_crit):
    if cells[p].nleaf >= n_crit:
        parts = []
        for c in range(8):
            if cells[p].nchild & (1 << c):
                parts.extend(get_particles(cells[p].child[c], cells, n_crit))
    else:
        parts = cells[p].leaf[:cells[p].nleaf]
    return parts
    ...
    if cells[p].nleaf >= n_crit:
        for c in range(8):
            if cells[p].nchild & (1 << c):
                ...
                # Ejecutar las siguientes lineas solo si no existe M2M
                for l in get_particles(cells[p].child[c], cells, n_crit):
                    dx = cells[p].x-particles[l].x
                    dy = cells[p].y-particles[l].y
                    dz = cells[p].z-particles[l].z
                    r    = np.sqrt(dx**2 + dy**2 + dz**2)
                    azim = np.arctan2(dy, dx)
                    incl = np.arccos(dz/r)

                    k = 0
                    for j in range(cells[p].l):
                        for m in range(-j, j+1):
                            cells[p].multipole[k] += np.sum(particles[l].m * r**j \
                                                           * np.conj(sph_harm(m,j,azim, incl)))
                            k += 1
                #--------------------------------------------------------------------
...
def M2M(p, c, cells):
    ...
    raise NotImplementedError("M2M not implemented")
        ...
        for octant in range(8):
            if cells[p].nchild & (1 << octant):
                ...
                # far-field child cell
                    ...
                    phi = 0
                    k = 0
                    for l in range(cells[c].l):
                        for m in range(-l,l+1):
                            phi += ((1/(2*l+1)) * sph_harm(m,l,azim, incl) / r**(l+1)) \
                                   * cells[c].multipole[k]
                            k += 1
                    
                    particles[i].phi += 4 * np.pi * phi.real
